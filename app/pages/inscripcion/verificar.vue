...
<template>
  <div class="min-h-screen bg-slate-100 text-slate-900 dark:bg-gradient-to-b dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 dark:text-white transition-colors">
    ...
    <main class="container mx-auto px-4 pt-20 sm:pt-24 pb-8 sm:pb-12 max-w-xl">
      <h1 class="text-xl sm:text-2xl font-bold mb-6 sm:mb-8 text-slate-900 dark:text-white">
        Verificar inscripción
      </h1>

      <div v-if="!token" class="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 sm:p-6">
      ...
      <div v-else-if="error || !registration" class="rounded-xl border border-red-500/30 bg-red-500/10 p-4 sm:p-6">
      ...
      <div v-else class="rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-4 sm:p-6 shadow-sm space-y-4">
        <div>
